...
function ReportesVentas() {
  ...
  const { data: totalRango, isLoading: isLoadingRango } = useGetVentasPorRango({ query: activeDates }, { query: { enabled: !!activeDates.fechaInicio, queryKey: ['ventas-rango', activeDates] } });
  const { data: porCliente, isLoading: isLoadingCliente } = useGetVentasPorCliente({ query: activeDates }, { query: { enabled: !!activeDates.fechaInicio, queryKey: ['ventas-cliente', activeDates] } });
  const { data: porVendedor, isLoading: isLoadingVendedor } = useGetVentasPorVendedor({ query: activeDates }, { query: { enabled: !!activeDates.fechaInicio, queryKey: ['ventas-vendedor', activeDates] } });
  const { data: porProducto, isLoading: isLoadingProducto } = useGetVentasPorProducto({ query: activeDates }, { query: { enabled: !!activeDates.fechaInicio, queryKey: ['ventas-producto', activeDates] } });
    ...
    <div className="space-y-8">
      <Card>
        <CardHeader className="bg-accent/10">
          ...
          <CardDescription>Selecciona un rango de fechas para actualizar todos los reportes.</CardDescription>
        </CardHeader>
        <CardContent className="pt-6">
          <div className="flex flex-col md:flex-row items-end gap-4">
            <div className="space-y-2 flex-1">
              <label className="text-sm font-medium">Fecha Inicio</label>
              <Input type="date" value={dates.fechaInicio} onChange={e => setDates({...dates, fechaInicio: e.target.value})} />
            </div>
            <div className="space-y-2 flex-1">
              <label className="text-sm font-medium">Fecha Fin</label>
              <Input type="date" value={dates.fechaFin} onChange={e => setDates({...dates, fechaFin: e.target.value})} />
            </div>
            <Button onClick={handleSearch} className="bg-primary hover:bg-primary/90 text-white w-full md:w-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <Card>
          ...
          <CardContent className="flex flex-col items-center justify-center py-8">
            {isLoadingRango ? <Skeleton className="w-32 h-12" /> : (
              <>
                <div className="text-muted-foreground mb-2 flex items-center gap-2">
                  <Calendar className="w-4 h-4" /> 
                  {totalRango?.fechaInicio} - {totalRango?.fechaFin}
                </div>
                <div className="text-5xl font-bold text-secondary-foreground">${totalRango?.totalVentas?.toFixed(2) || '0.00'}</div>
              </>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            {isLoadingCliente ? <Skeleton className="w-full h-40" /> : (
                  ...
                  <TableBody>
                    {porCliente?.map(c => (
                      <TableRow key={c.idCliente}>
                        ...
                        <TableCell className="text-right font-medium">${c.totalCliente.toFixed(2)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
        ...
        <Card>
          ...
          <CardContent>
            {isLoadingVendedor ? <Skeleton className="w-full h-40" /> : (
              <div className="overflow-x-auto max-h-64">
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {porVendedor?.map(v => (
                      <TableRow key={v.idVendedor}>
                        ...
                        <TableCell className="text-right font-medium">${v.totalVendedor.toFixed(2)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
        ...
        <Card>
          ...
          <CardContent>
            {isLoadingProducto ? <Skeleton className="w-full h-40" /> : (
              <div className="overflow-x-auto max-h-64">
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {porProducto?.map(p => (
                      <TableRow key={p.idProducto}>
                        ...
                        <TableCell className="text-right font-medium">${p.totalVendido.toFixed(2)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
